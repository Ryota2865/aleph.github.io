...
## 0.7.20-43 (2026-08-06) — ブックリスト座標系カタログのオーナー採用（文書のみ）

2026-08-05のChat対話伝書（`reports/FABLE5_CHAT_BOOKLIST_COORDINATE_SPACE_20260805.md`）
に対する設計査読 `designs/booklist-coordinate-catalog.md` の判定三件を、オーナーが
2026-08-06に全面採用した（「すべて採用します」）。判定と配線は以下のとおり。

1. **案1（L1への文脈注入）= 既定注入不可を確定**。§14-6（外部の錨は既定で入れない）
   に基づき、カタログは著者（author）が自ら要請したときに提供できる資料棚としてのみ
   登録する。プロンプトへの恒常注入は行わない。
2. **案2（査読基準の系譜宣言）= 向きを反転して採用**。poetics.mdは自己生成正典
   （§14-10）のため、カタログをその依拠先として与えず、観測点明記の外部観測
   （観測点 = Ryota × Claude Fable 5 Chat, 2026-08-05）として批評パケットの
   任意添付にする。配線として`designs/critic-role.md` §1.1へ任意添付規則を追加
   （パケット構成規則の変更。§12.2の設計者審査を本項で記録）。カタログへの適合・
   距離を評価基準にすることは禁止する。
3. **案3（埋め込み空隙実験）= 事前登録草案を記録し、実施は2026-10審査へ留保**。
   0.7.20-40第5項（潜在空間系計器の10月留保）と§14.1（8–9月の非blocking新機構凍結）
   に従う。`designs/post-phase6-aug-sep-literary-campaign.md` §5「2026-10」へ
   審査項目として追記。カタログ距離の目的関数化は時期を問わず禁止
   （§16.1・Goodhart・model-collapse対策）。

等級カタログv0（tier1=21・観測点依存の等級・関係線・開放枠）の正本は設計文書§3とし、
機械可読版（`corpus/catalog/booklist.v1.json`想定）は10月審査の案3承認に連動させる。
本項は文書のみの変更である。PLAN.md本文、poetics/、config、有償call、local inference、
実装の変更はない。既存の監査済みcandidate（`audit-candidate/*` tag）は変更しない。
...
read-only正式監査した。欠落RED、空履歴、有効履歴、malformed、dir欠落を独立注入し、
P0〜P3なし、**VERDICT: PASS**。R-8をformal完了する。候補はproof commit
`d4654f86e1fc8d7c751b78a5f23305f797d3821a`と
`audit-candidate/phase6-missing-poetics-history-20260725` tagへ固定した。
...
同じ監査担当が修繕tree `9940cbb4062187ce93fd4cdc394ecc98b52b522c`をread-onlyで
focused再監査し、実Git timeoutを使ったHEAD/INDEX両経路のRED→GREEN、29＋3 probe、
focused **21 passed**、全non-local **426 passed, 1 deselected**を独立再現した。
P0〜P2なし、**VERDICT: PASS**としてR-2をformal完了する。

初回FAIL treeとPASS treeはそれぞれproof commit/tagへ固定し、FAIL証拠のGC耐性に関する
P3-5をclosureで解消した。P3-1〜P3-4、INDEX経路の恒久RED test不足（P3-6）、
Git index flag等に関する監査uncertaintyは残置する。新作、local inference、有償API call、
...
2026-07-25、初回FAILを発行した同一Claude Code担当が修繕tree
`351cd83ffe0bdaeec527d3dbb512e63070ccd19e`をread-only focused再監査した。開始・終了時の
tree一致、unstaged/untracked 0件、focused 16件、全non-local 421 passed, 1 deselected、
独立故障注入を再現し、F-1〜F-7を全件閉鎖、P0/P1なし、**VERDICT: PASS**。原文は
`reports/PHASE6_CURRENT_STATE_P1_P2_REAUDIT_20260725.md`に保存した。

残置リスクはR-1〜R-9として保持する。とくに、ledgerの`supersedes`検証がJSON記載順へ依存する
fail-closedなR-1と、currencyがcandidate tree実体ではなくCHANGELOG番号にのみ束縛されるR-2を、
次tracer bulletの優先候補とする。今回のformal PASSがこれらを解消したとは扱わず、closureでは
コード・test・契約を変更しない。
