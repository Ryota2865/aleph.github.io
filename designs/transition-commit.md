...
`strict_replay()`はevent id、command id一意性、遷移元連続性、正典遷移、payload型を
検証し、一つでも破れば停止する。矢印の右側だけを採用しない。

0.7.20-5以前のL0行はlegacyとしてread-only監査できるが、strict streamとはみなさない。
既存行の削除・並べ替え・書換えは禁止する。legacy作品へ新しい状態操作が必要な場合は、
現在のcheckpointと不一致一覧を明示した`reconciliation` eventを追記し、そこを新しい
厳密再生区間の基点とする。reconciliationは過去が整合していたという主張ではない。
