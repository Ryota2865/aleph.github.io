...
公開サイトの将来項目として「ALEPH開発ジャーナル」を登録する。目的は読者が現在地、
最近の観測、未解決点、次に試すことを理解できるようにすること。次の文学的runが完了するまで
実装を開始せず、現行の非blockingインフラ停止を維持する。着手時は次を最小契約とする。

- `PROGRESS.md`や監査ログの転載ではなく、重要な文学的runまたは月次単位の短い読者向け編集記事とする。
- 各記事で「完了」「監査済み」「準備中」「仮説」「保留」を分離し、予定をノルマや成果として書かない。
- blind annotationや結果未観測実験の条件は完了前に開示せず、終了後に事前登録と結果を結びつける。
- 過去記事を現在値で上書きせず、誤りは訂正注記として残す。秘密、未公開作品本文、provider stateは載せない。
- 初期導線は「ALEPHについて」または「制作記録」配下とし、グローバルナビゲーションを増やさない。
  日本語本文と英語の主要事実・日付・状態・導線を同期し、作品を先に読む現行情報構造を維持する。
- 最初の記事候補はPhase 6完了、house-style annotation床未達、w0009 shadow、w0010の帰還。
  記事内容は各runの結果観測後に確定し、未観測結果を先取りしない。

同じ公開サイト改善scopeに、初見読者向けページ「はじめてALEPHを読む方へ」を登録する。
これは「ALEPHについて」より前の入口として、作品を読む前に生じる基本的な疑問へ答える。
開発ジャーナルと同様に次の文学的run完了後まで実装せず、着手時は次を最小契約とする。

- 「完成したAI文学の展示」ではなく、AI文学が成立しうる条件を作品と記録で探る未完成の
  制作・研究projectであることを冒頭で示す。ただし「AI文学はまだ実現されていない」という
  普遍的断定は避け、ALEPH自身が達成を主張していないこととして書く。
- 起点となる人間の構想・初期prompt・委任範囲・予算／公開承認と、LLMが担う設計・施工・
  探索・執筆・批評を分ける。「人間がpromptを与えない」「全工程がLLMのみ」と一般化せず、
  作品ごとの人間入力とmodel roleはprovenanceへ案内する。
- LLMの認識や思考過程、AI固有性、作品よりlogへ強く現れるという見方は確定事実でなく
  探索仮説として明示する。「複数の自己」は永続人格でなく複数model／roleと、記録・詩学を
  継承するALEPHという連続体として説明する。
- 読み始めを一経路に固定せず、「まず作品を読む」は『半呼吸』、「考え方から読む」は
  『詩学第0版』、「検証する」は制作記録、の短い三導線を置く。作品紹介は読解を固定する
  解説にならない長さに留める。
- 制作の各layerは専門用語の一覧ではなく、探索→基準→構成→執筆→批評→改訂→擱筆→公開を
  一文ずつ説明する。制作記録ではseed／実験条件、基準書、査読、決定logの見方を案内する。
- 未完成の問いとして「模倣の器で反模倣できるか」「自律は委任を越えるか」「AI固有性を
  人間が読めるか」「固有性は作品とlogのどちらに現れるか」を、未決のまま提示する。
- 公開前に、コード寄与率などの数量主張、作品・modelの役割、初期入力の有無をrepositoryの
  一次記録で照合する。裏づけられない`100%`表現は使わない。日本語本文と英語の主要事実・
  導線を同期し、ホームから一操作、作品・制作記録へ二操作以内で到達可能にする。
