...
- **Phase2（英語ミラー）codex実装中**: 仕様 state/tasks/M8_phase2_en_codex_task.md、
  結果 state/tasks/M8_phase2_result.md（完了時）、ログ state/tasks/M8_phase2_run.log。
  対象は scripts/build_public_site.py のみ（allowlist）。docs/en/{index,about,dialogue,poetics,ode}.html +
  en/works/w0004.html + en/research/index.html を生成、JP⇄EN言語スイッチ、作品は英語枠組み+原文リンク
  （全文翻訳しない）。
- **codex完了後の手順**: (1) diff精査（英文品質・研究主張の忠実性: D=installs not detects,
  E=良品で文面頑健, border=質に錨づく, C=詩学/モデル非依存・増幅器は要追試） (2) uv run python
  scripts/build_public_site.py で再生成（border-E報告が研究ページに自動収載される） (3) pytest 138緑
  (4) commit+push (5) PROGRESS更新。
