...
- **モデル表記の未決**: オーナー記録は「Claude 3.5 Sonnet」だが対話日（2024-04-21）は
  3.5 Sonnet公開（2024-06-20）より前。新設ページは「Claude」表記に留めた。
  ODE.md本文（オーナーの文書）の3.5表記は未変更。**7/19前にFable5チャットの
  会話履歴検索で対話の実日付・引用文の正確な出所を検証することを推奨**（オーナー宛）。
