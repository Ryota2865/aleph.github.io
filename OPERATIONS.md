...
## 設計変更の門（オーナー採用 2026-07-17、出典: sol）

新しい仕様変更・機構追加を提案・着手する前に、一つの問いを置く:

> **この変更は、実際の作品または実験で観測された失敗に応答しているか。**
> 答えが「いいえ」なら、急いで実装しない。

ALEPHには十分な骨格がある。重要なのは骨格をさらに複雑にすることではなく、
その中から予想できなかった作品と問いが本当に生まれるかを見ることである。
（オーナー: 「本質的でない仕様変更をしないためのガイドライン」。司令塔・実装
ワーカーは、この門を通らない変更の依頼を受けたら、着手前にこの節を引いて
オーナーへ差し戻してよい。）
